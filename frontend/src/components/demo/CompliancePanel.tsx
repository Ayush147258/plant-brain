...
export function CompliancePanel({ onComplete }: { onComplete: () => void }) {
  ...
  const refresh = async () => {
    ...
    setError('');
    try {
      const [rulesResponse, docsResponse] = await Promise.all([listComplianceRules(), listDocuments(0, 100)]);
      setRules((rulesResponse.rules || []) as ComplianceRule[]);
      setDocuments(docsResponse.documents || []);
    } catch (err: any) { setError(err?.status ? `${err.message} (HTTP ${err.status})` : err?.message || 'Failed to load compliance data.'); }
