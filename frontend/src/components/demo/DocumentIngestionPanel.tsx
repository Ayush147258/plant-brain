...
function friendlyProcessingMessage(message: string) {
  const lowered = String(message || '').toLowerCase();
  if (lowered.includes('503') || lowered.includes('unavailable') || lowered.includes('high demand') || lowered.includes('model is currently')) {
    return 'Gemini structured extraction is temporarily busy. PlantBrain will still index the document text and citations; retry the demo document to resume without the raw model error.';
  }
  if (lowered.includes('gemini structured extraction is temporarily busy')) {
    return 'Gemini structured extraction is temporarily busy. PlantBrain indexed the document text and citations for Q&A.';
  }
...
function statusClasses(status: string) {
  ...
  if (normalized === 'completed') return 'border-emerald-400/20 bg-emerald-400/10 text-emerald-100';
  if (normalized === 'failed') return 'border-red-400/25 bg-red-400/10 text-red-100';
...
export function DocumentIngestionPanel({ onComplete, onStatsChange }: { onComplete: () => void; onStatsChange: (count: number) => void }) {
  ...
  const refresh = useCallback(async (showLoader = false) => {
    ...
    try {
      ...
      setError(friendlyProcessingMessage(err?.status ? `${err.message} (HTTP ${err.status})` : err?.message || 'Failed to load documents.'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const track = async (result: UploadResponse, fallbackFilename: string) => {
    ...
    for (let attempt = 0; attempt < 45; attempt += 1) {
      ...
      const rawMessage = status === 'completed'
        ? 'Indexed and ready for questions'
        : status === 'failed'
          ? String(current.error_message || 'Processing failed')
          : 'Backend is parsing and indexing this file';
      setActivity({
        id,
        filename: String(current.original_filename || current.filename || filename),
        status,
        chunks: Number(current.total_chunks || 0),
        message: friendlyProcessingMessage(rawMessage),
      });
      await refresh(false);
  ...
  const upload = async () => {
    ...
    try {
      ...
      const message = friendlyProcessingMessage(err?.status ? `${err.message} (HTTP ${err.status})` : err?.message || 'Upload failed.');
      setError(message);
  ...
  const useDemoPdf = async () => {
    ...
    try {
      await track(await loadDemoDocument(), 'OSHA_3120_Lockout_Tagout.pdf');
    } catch (err: any) {
      const message = friendlyProcessingMessage(err?.message || 'Could not load the demo PDF.');
      setError(message);
      setActivity((current) => current ? { ...current, status: 'failed', message } : current);
    } finally {
  ...
  return <div className="space-y-5">
    <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/20">
      <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300">Step 01 - Connect your documents</div>
          <h2 className="mt-2 text-2xl font-semibold text-white">Upload real plant files</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Select a file, confirm its name, then watch it move through upload, parsing, and indexing.</p>
        </div>
        <button onClick={() => refresh(true)} className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-sm text-slate-200 hover:border-cyan-400/50"><RefreshCw className="h-4 w-4" /> Refresh</button>
      </div>
      {error && <div className="mb-4 rounded-xl border border-red-400/20 bg-red-400/10 p-3 text-sm text-red-100">{error}</div>}
      {activity && <div className={`mb-4 flex items-start gap-3 rounded-xl border p-4 ${activityTone}`}>
        <ActivityIcon className={`mt-0.5 h-5 w-5 shrink-0 ${!isTerminal(activity.status) ? 'animate-spin text-cyan-300' : activity.status === 'completed' ? 'text-emerald-300' : 'text-red-300'}`} />
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="break-all font-semibold text-white">{activity.filename}</span>
            <span className="rounded-full border border-white/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-200">{activity.status}</span>
          </div>
          <p className="mt-1 text-xs text-slate-300">{activity.message}{activity.chunks ? ` | ${activity.chunks} indexed chunks` : ''}</p>
          {activity.status === 'failed' && <button onClick={useDemoPdf} disabled={loadingDemo || uploading} className="mt-3 rounded-lg border border-red-300/20 bg-red-300/10 px-3 py-2 text-xs font-semibold text-red-50 hover:border-red-200/40">Retry demo indexing</button>}
          {activity.id && <p className="mt-1 truncate font-mono text-[10px] text-slate-500">Document ID: {activity.id}</p>}
        </div>
      </div>}
