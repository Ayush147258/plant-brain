    ...
    <div className="space-y-2">
      {items.map((source, index) => (
        <div key={index} className="rounded-xl border border-white/10 bg-white/[0.04] p-3">
          ...
          <div className="mt-1 text-xs text-slate-400">{source.section || source.page_or_section || (source.chunk_index !== undefined ? `Chunk ${source.chunk_index}` : 'Referenced context')}</div>
