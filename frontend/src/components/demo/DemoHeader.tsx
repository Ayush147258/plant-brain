...
export function DemoHeader({ currentStep, totalSteps, completedCount, documentsCount, equipmentCount }: { currentStep: number; totalSteps: number; completedCount: number; documentsCount: number; equipmentCount: number }) {
  return (
    <header className="border-b border-white/10 bg-[#0a0f19]/95 px-6 py-6 backdrop-blur">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold leading-tight tracking-tight text-white">PlantBrain Interactive Demo</h1>
          <p className="mt-2 text-sm leading-6 text-slate-400">Run a real backend-connected plant intelligence workflow</p>
        </div>
